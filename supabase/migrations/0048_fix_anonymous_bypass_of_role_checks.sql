-- 0048_fix_anonymous_bypass_of_role_checks.sql
-- `current_user_role() not in ('staff', 'manager', 'admin')` returns
-- NULL (not TRUE) for a fully anonymous caller -- current_user_role()
-- itself returns NULL when auth.uid() is null (no profile row to
-- resolve), and `NULL not in (...)` is NULL under three-valued logic.
-- A plpgsql `IF <condition> THEN` treats a NULL condition as FALSE, so
-- `if current_user_role() not in (...) then raise exception` NEVER
-- raises for an anonymous/unauthenticated caller -- only for a
-- logged-in caller whose role genuinely isn't in the list. Every
-- function using this exact pattern as its ONLY authorization gate is
-- fully bypassable by an anonymous request using nothing but the
-- public anon key. Verified live: `select (null::user_role not in
-- ('staff','manager','admin'))` returns NULL.
--
-- This affects 3 pre-existing functions (found during a full-webapp
-- security review, 2026-07-21) plus 3 introduced by this review's own
-- earlier fixes today, all corrected here with the same fix -- an
-- explicit `current_user_role() is null or ... not in (...)` check:
--
--   - fulfill_redemption: an anonymous caller could mark any customer's
--     unused reward-redemption code as fulfilled, denying them the
--     reward, with no staff session at all.
--   - get_staff_members: an anonymous caller could dump every staff/
--     manager/admin's name, phone, and email (joined from auth.users)
--     -- a real PII leak of employee data.
--   - reply_to_review: an anonymous caller could post a fake "official"
--     reply on any customer review.
--   - get_tables_admin (introduced in 0046): an anonymous caller could
--     read the qr_code_token column 0046/0047 were specifically trying
--     to lock down, via this RPC's own bypassed role check.
--   - regenerate_table_qr_token (rewritten in 0046): an anonymous
--     caller could regenerate (invalidate) any table's QR token.
--   - place_order (fixed in 0044): a fully anonymous caller (not just
--     a logged-in non-staff customer) could still set
--     paymentCollected: true and get an order marked paid for free --
--     the exact vulnerability 0044 was meant to close was only half
--     closed.
...
returns table(id uuid, full_name text, phone text, role user_role, is_active boolean, email text)
language plpgsql
security definer
set search_path = public
as $$
...
as $$
begin
  if public.current_user_role() is null or public.current_user_role() not in ('manager', 'admin') then
